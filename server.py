...
def simulate_emergency():
    ...
    predefined = [
        {'emergency_id': 'E001', 'hospital_id': 'h1', 'required_blood_type': 'A+', 'units_required': 5},
        {'emergency_id': 'E002', 'hospital_id': 'h2', 'required_blood_type': 'O-', 'units_required': 3},
        {'emergency_id': 'E003', 'hospital_id': 'h3', 'required_blood_type': 'B+', 'units_required': 7},
        {'emergency_id': 'E004', 'hospital_id': 'h1', 'required_blood_type': 'AB-', 'units_required': 2},
        {'emergency_id': 'E005', 'hospital_id': 'h2', 'required_blood_type': 'O+', 'units_required': 6}
    ]

    emergency = random.choice(predefined)
    add_log(f"🚨 Simulated Emergency: {emergency['emergency_id']} at {emergency['hospital_id']}", 'emergency')
    add_log(f"Need {emergency['units_required']} units of {emergency['required_blood_type']}", 'info')
    ...
    if not result['sources']:
        add_log(f"❌ No {emergency['required_blood_type']} blood found", 'warning')
    else:
        lives_saved = sum([s['units'] for s in result['sources']])
        add_log(f"✅ {lives_saved} units secured from {len(result['sources'])} source(s)", 'success')
